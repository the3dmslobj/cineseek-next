      ...
      <div className="mx-auto flex lg:flex-row flex-col w-full xl:w-265 flex-1 overflow-hidden lg:px-20 xl:px-0">
        <div className="lg:w-57.5 w-full flex flex-col">
          ...
          <div className="flex flex-col ml-10 my-5 lg:my-0 md:ml-20 lg:ml-0">
            <div className="flex w-full md:w-57.5 flex-wrap mt-2">
            ...
            <div className="flex w-full md:w-57.5 flex-wrap">
              {isTv ? (
                <>
              ...
              >
                {credits.cast.map((c) => (
                  <Link
                    href={`/person/${c.id}`}
                    key={c.id}
                    className="m-2 w-25 shrink-0 cursor-pointer"
                  >
                    <img
                      className="w-25 h-25 object-cover object-top rounded"
                      src={
                        c.profile_path
                          ? `${TMDB_IMG}${c.profile_path}`
                          : FALLBACK_PROFILE
                      }
                      alt=""
                    />
...
function SectionTitle({ children }: { children: React.ReactNode }) {
  return (
    <div className="text-2xl md:text-3xl font-bold text-color3">{children}</div>
  );
}
